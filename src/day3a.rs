...
fn fillmap(
    ...
    for item in v {
        ...
        for _i in 0..item.dist() {
            pos.0 += d.x();
            pos.1 += d.y();
            *map.entry(pos).or_insert(0) |= wireidx;
        }
    }
    map
}

fn find_min_dist_intersection(map: BTreeMap<(i32, i32), i32>) -> i32 {
    let mut mindist = 99999999;
    for (key, val) in &map {
        if *val == 3i32 {
            let pos = key;
            let md = pos.0.abs() + pos.1.abs();
            if 0 != md && md < mindist {
                mindist = md;
            }
        }
...
mod tests {
    ...
    fn test_add() {
        ...
        let mindist = find_min_dist_intersection(map);

        assert_eq!(135, mindist);
    }
}
