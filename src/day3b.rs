...
fn str2t(s: String) -> Segment {
    let ns = &s[1..s.len()];
    let dist = ns.parse::<u32>().unwrap();

    match s.chars().nth(0).unwrap() {
        'R' => return Segment::new(Direction::new(1, 0), dist),
        'L' => return Segment::new(Direction::new(-1, 0), dist),
        'U' => return Segment::new(Direction::new(0, -1), dist),
        'D' => return Segment::new(Direction::new(0, 1), dist),
        _ => panic!(),
    }
}
...
fn fillmap(
    wire: &str,
    wireidx: i32,
    mut map: BTreeMap<(i32, i32), Point>,
) -> BTreeMap<(i32, i32), Point> {
    let v: Vec<_> = wire
        .split(',')
        .map(|item| str2t(item.to_string()))
        .collect();
    let mut pos = (0, 0);
    let mut wire_len = 0;
    for item in v {
        let d = item.dir();
        for _i in 0..item.dist() {
            pos.0 += d.x();
            pos.1 += d.y();
            wire_len += 1;
            if 1 == wireidx {
                map.entry(pos)
                    .and_modify(|item| {
                        *item = Point::new(item.mark() | wireidx, wire_len, item.dist2())
                    })
                    .or_insert(Point::new(wireidx, wire_len, -1));
            } else {
                map.entry(pos)
                    .and_modify(|item| {
                        *item = Point::new(item.mark() | wireidx, item.dist1(), wire_len)
                    })
                    .or_insert(Point::new(wireidx, -1, wire_len));
            }
        }
    }
    map
...
fn find_min_dist_intersection(map: BTreeMap<(i32, i32), Point>) -> i32 {
    let mut mindist = 99999999;
    for (_key, val) in &map {
        if val.mark() == 3i32 {
            let d = val.dist1() + val.dist2();
            if 0 != d && d < mindist {
                mindist = d;
            }
        }
...
fn day3b(w1: &str, w2: &str) -> i32 {
    let mut map: BTreeMap<(i32, i32), Point> = BTreeMap::new();
    map = fillmap(w1, 1, map);
    map = fillmap(w2, 2, map);

    find_min_dist_intersection(map)
}
